export function CreatorAgreementContent() {
  return (
    <>
      <p>
        This Creator Agreement (&quot;Agreement&quot;) is entered into between you
        (&quot;Creator&quot;) and Foleio (&quot;Platform,&quot; &quot;we,&quot;
        &quot;us&quot;). By completing creator onboarding and activating your
        creator account, you agree to be bound by this Agreement, our Terms of
        Service, and our Privacy Policy.
      </p>

      <h2>1. Creator Account</h2>
      <p>
        To become a creator on Foleio, you must complete onboarding, provide a
        valid Nigerian bank account registered in your name, agree to
        Paystack&apos;s terms of service, and be at least 18 years of age. Foleio
        may require additional identity verification at any time to comply with
        AML regulations or Paystack&apos;s KYC requirements.
      </p>

      <h2>2. Your Content</h2>
      <p>
        <strong>Ownership:</strong> You retain full ownership of all content you
        upload to Foleio. Nothing in this Agreement transfers ownership of your
        content to Foleio.
      </p>
      <p>
        <strong>Licence to Foleio:</strong> By uploading content, you grant us a
        non-exclusive, royalty-free, worldwide licence to host, store, stream,
        display, and deliver your content to fans and customers who have
        legitimately purchased or subscribed to access it, and to generate
        thumbnails and previews for your public profile. This licence terminates
        when you delete the content or close your account.
      </p>
      <p>
        <strong>Content Standards:</strong> All content must be owned by you or
        properly licensed, accurately represent what fans are paying for, comply
        with all applicable Nigerian laws, and not infringe on any third-party
        rights or contain harmful, illegal, or obscene material. Foleio reserves
        the right to remove any content that violates these standards without
        prior notice.
      </p>

      <h2>3. Monetization &amp; Revenue</h2>
      <p>
        <strong>Revenue Streams:</strong> As a creator you may earn through
        service bookings (full payment or deposit), shop / product sales, paid
        content and tutorials, and other features Foleio makes available on your
        profile.
      </p>
      <p>
        <strong>Platform Fees (bookings, shop, and most paid transactions):</strong>{' '}
        Foleio deducts a platform fee from the gross customer payment based on
        your platform plan:
      </p>
      <ul>
        <li>
          <strong>Free plan:</strong> 5% of each transaction at or above ₦5,000.
          For charges under ₦5,000, Foleio charges a flat ₦300 platform fee
          (capped at the charge amount) instead of 5%.
        </li>
        <li>
          <strong>Pro plan (₦10,000/month):</strong> 0% Foleio platform fee on
          those transactions while your Pro subscription is active.
        </li>
      </ul>
      <p>
        After the Foleio platform fee, you keep the remaining creator share
        (95% on Free for standard-sized charges, or 100% on Pro, before any
        payment-processor fees described below). Confirmed customer payments are
        settled to your linked bank account as described under Settlement.
      </p>
      <p>
        <strong>Payment processing:</strong> Paystack&apos;s processing fees are
        separate from Foleio&apos;s platform fee and are deducted or settled
        according to Paystack&apos;s terms at the point of transaction. Even on
        Pro (0% Foleio fee), Paystack fees still apply.
      </p>
      <p>
        <strong>Deposits &amp; balances:</strong> You may offer deposit-based
        bookings. Deposits and remaining balances are subject to the same
        platform fee rules as other bookings. Unpaid balances remain the
        customer&apos;s obligation under the booking terms you set.
      </p>
      <p>
        <strong>Your pricing:</strong> You set your own service, shop, and
        content prices. By listing an offer, you represent that the price and
        included benefits are accurate.
      </p>

      <h2>4. Settlement</h2>
      <p>
        Foleio does not operate a separate creator withdrawal or payout-request
        wallet. Your creator share is settled <strong>directly to your
        registered Nigerian bank account</strong> via Paystack subaccount
        settlement, on Paystack&apos;s normal schedule (typically the next
        business day after a successful charge, subject to weekends, bank
        holidays, and Paystack&apos;s processing).
      </p>
      <p>
        You must keep a verified bank account in your name linked on the
        Platform. Settlement timing and banking rails are controlled by Paystack
        and your bank; Foleio does not guarantee same-day credit.
      </p>
      <p>
        <strong>Settlement holds:</strong> Foleio and/or Paystack may delay or
        hold settlement if a dispute or refund claim is pending, if fraudulent
        activity is suspected, if required by law, or if your identity or bank
        account cannot be verified. We will notify you of any hold we control.
      </p>

      <h2>5. Creator Obligations &amp; Conduct</h2>
      <p>As a creator, you agree to:</p>
      <ul>
        <li>
          Respond to booking inquiries and service requests in a timely manner;
        </li>
        <li>
          Deliver booked services on the agreed date and as described in your
          services list;
        </li>
        <li>
          Honour your published cancellation and refund policy for customer
          cancellations;
        </li>
        <li>
          Notify customers promptly if you are unable to fulfil a booking and
          cooperate with Foleio to facilitate a refund;
        </li>
        <li>Not artificially inflate views, subscribers, or engagement metrics;</li>
        <li>
          Not solicit payments from fans outside the Platform to circumvent
          Foleio&apos;s fees;
        </li>
        <li>Not use the Platform to promote or sell illegal goods or services;</li>
        <li>
          Maintain professional conduct in all interactions with fans, customers,
          and Foleio staff.
        </li>
      </ul>

      <h2>6. Platform Plans &amp; Subscription</h2>
      <p>
        Foleio currently offers two creator platform plans:
      </p>
      <ul>
        <li>
          <strong>Free (₦0):</strong> Full access to bookings, shop, services,
          and creator tools, with a <strong>5%</strong> Foleio fee on qualifying
          transactions (or <strong>₦300 flat</strong> on charges under ₦5,000).
          Paystack processing fees still apply.
        </li>
        <li>
          <strong>Pro (₦10,000/month):</strong> Everything on Free, with{' '}
          <strong>0%</strong> Foleio platform fee on bookings, shop, and other
          covered transactions while the subscription is active. Only Paystack
          processing fees apply to those charges.
        </li>
      </ul>
      <p>
        Pro is billed monthly in advance through Paystack. You may cancel at any
        time; your Pro benefits remain active until the end of the current paid
        period, after which your account returns to Free and the 5% / ₦300 fee
        schedule resumes. Foleio reserves the right to change platform
        subscription pricing with 30 days&apos; advance notice to existing
        subscribers.
      </p>
      <p>
        Legacy or invite-only Premium accounts, if any, are treated like Pro for
        platform fee purposes (0% Foleio fee while active) unless we notify you
        otherwise.
      </p>

      <h2>7. Cancellations &amp; Refunds</h2>
      <p>
        You may configure a cancellation policy for bookings. Unless you set a
        different policy, Foleio&apos;s default tiers apply: full refund when
        cancelled 60+ days before the booking date; 50% refund when cancelled
        30–59 days before; and no refund when cancelled fewer than 30 days
        before. Refunds are calculated on amounts already paid by the customer
        and are subject to payment-processor rules and this Agreement.
      </p>

      <h2>8. Intellectual Property</h2>
      <p>
        Foleio&apos;s name, logo, platform design, and all associated technology
        are the exclusive intellectual property of Foleio. Profiles and checkout
        experiences may display Foleio attribution. You may not use Foleio
        trademarks except as needed to identify the Platform.
      </p>

      <h2>9. Account Suspension &amp; Termination</h2>
      <p>
        <strong>By Foleio:</strong> We may suspend or terminate your account,
        with or without notice, for breach of this Agreement, repeated content
        violations, fraudulent activity, non-cooperation with KYC requests, or
        any conduct harmful to users or the Platform.
      </p>
      <p>
        <strong>By You:</strong> You may close your account at any time by
        contacting legal@foleio.com. Amounts already settled or in progress with
        Paystack for legitimate transactions completed before closure will follow
        normal settlement.
      </p>
      <p>
        <strong>Effect of Termination:</strong> Upon termination, your public
        profile will be deactivated and your content removed. Customers with
        pending bookings or orders will be notified as appropriate. Foleio is not
        liable for any loss of earnings resulting from account termination for
        cause.
      </p>

      <h2>10. Representations &amp; Warranties</h2>
      <p>
        You represent and warrant that you have the full legal right to enter into
        this Agreement, all content you upload is owned by you or properly
        licensed, you will comply with all applicable Nigerian laws including tax
        laws, the bank account you register is legitimately held in your name, and
        you are not subject to any sanctions or legal prohibitions that would
        prevent you from using the Platform.
      </p>

      <h2>11. Limitation of Liability</h2>
      <p>
        Foleio&apos;s liability to you under this Agreement is limited to the fees
        paid by you to Foleio in the three months preceding any claim. Foleio is
        not liable for loss of earnings, loss of content, or any indirect damages
        arising from Platform downtime, payment processing failures, or
        termination of your account for cause.
      </p>

      <h2>12. Amendments</h2>
      <p>
        Foleio may amend this Agreement at any time with 14 days&apos; notice by
        email. Your continued use of the Platform after the effective date
        constitutes acceptance. If you do not accept an amendment, you may close
        your account before the effective date.
      </p>

      <h2>13. Governing Law</h2>
      <p>
        This Agreement is governed by the laws of the Federal Republic of Nigeria.
        Any disputes shall be resolved by arbitration in Lagos, Nigeria, in
        accordance with the Arbitration and Conciliation Act.
      </p>

      <h2>14. Contact</h2>
      <p>Email: legal@foleio.com</p>
    </>
  );
}
