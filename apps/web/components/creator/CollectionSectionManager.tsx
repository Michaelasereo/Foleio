...
export function CollectionSectionManager({
  ...
  useEffect(() => {
    ...
    setUnsectionedOrderIds((prev) => {
      if (prev.length === 0) {
        return [...unsectionedVideos]
          .sort((a, b) => {
            const orderA = a.sectionOrder ?? 0;
            const orderB = b.sectionOrder ?? 0;
  ...
  const persistUnsectionedOrder = async (orderedIds: string[]) => {
  ...
  const reorderUnsectioned = async (orderedIds: string[]) => {
    setUnsectionedOrderIds(orderedIds);
    try {
      await persistUnsectionedOrder(orderedIds);
      toast({
        title: 'Order saved',
        description: 'Your collection order has been updated',
      });
      router.refresh();
    } catch (error) {
      toast({
  ...
  const handleSort = async (sortBy: string) => {
    ...
    let sorted = [...orderedUnsectionedVideos];
    switch (sortBy) {
      case 'newest':
        sorted.sort(
          (a, b) =>
    ...
    await reorderUnsectioned(sorted.map((video) => video.id));
  };
    ...
    <div className="space-y-4">
        ...
        <div className="mb-4 space-y-1 rounded-xl border border-border/80 bg-muted/20 p-3">
          <div className="mb-3 flex items-center justify-between">
            <p className="text-sm text-muted-foreground">
              {unsectionedVideos.length} video{unsectionedVideos.length !== 1 ? 's' : ''} · drag to reorder
            </p>
            <div className="flex items-center gap-2">
              <span className="text-xs text-muted-foreground">Sort by:</span>
              <select
                value={selectedSort}
                onChange={(event) => void handleSort(event.target.value)}
      ...
      <Dialog open={showAddVideoModal} onOpenChange={setShowAddVideoModal}>
        <DialogContent className="max-w-md">
          <DialogTitle className="sr-only">Add video to section</DialogTitle>
          <DialogDescription className="sr-only">
            Select one of your videos to add it to this collection section.
          </DialogDescription>
