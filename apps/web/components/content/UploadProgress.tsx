  ...
  return (
    <div className="w-full rounded-2xl border border-border bg-white p-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          {state === 'uploading' && speed && (
            <span className="text-xs text-muted-foreground">{speed}</span>
          )}
          {state === 'uploading' && timeRemaining && (
      ...
      {state === 'processing' && (
        <div className="mt-3 rounded-xl bg-amber-50 p-3">
          <p className="text-xs text-amber-800">
            Your video uploaded successfully. Mux is now processing it - this usually takes 1-3
            minutes depending on file size. You can leave this page and return shortly.
          </p>
        </div>
      )}
      ...
      {state === 'complete' && (
        <div className="mt-3 rounded-xl bg-green-50 p-3">
          <p className="text-xs text-green-800">Your video is live and ready for your audience.</p>
        </div>
      )}
