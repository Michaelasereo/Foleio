...
import { Loader2, Save, Send, Share2, Upload } from 'lucide-react';
...
export function JournalEditor({
  ...
  async function saveDraft() {
    ...
    setIsSaving(true);
    try {
      const currentContent = editorInstance?.getHTML?.() ?? content;
      const tags = tagsText
        .split(',')
        .map((tag) => tag.trim())
        .filter(Boolean);
  ...
  async function handleUploadCover(file: File) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(result.details || result.error || 'Image upload failed');
      }
      setCoverImage(result.url);
      toast({
        title: 'Cover uploaded',
        description: 'Your journal cover image is ready.',
      });
    } catch (error: any) {
      toast({
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        ...
        <p className="text-xs text-muted-foreground">
          JPG, PNG or WebP · Max 5MB · Minimum 1200×675px (16:9 recommended)
        </p>
      </div>
