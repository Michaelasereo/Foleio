...
export async function PATCH(
  ...
  try {
    ...
    // Keep one section assignment per collection.
    ...
    let unsectionedSortOrder: number | undefined;
    if (!body.sectionId) {
      const sectionContentIds = await prisma.sectionContent.findMany({
        where: {
          section: {
      ...
      const sectionedIds = sectionContentIds.map((item) => item.contentId);
      unsectionedSortOrder = await prisma.content.count({
        where: {
          collectionId: body.collectionId,
          id: {
            not: contentId,
    ...
    // Keep content marked as collection content.
    await prisma.content.update({
      where: { id: contentId },
      data: {
        collectionId: body.collectionId,
        ...(unsectionedSortOrder !== undefined ? { sectionOrder: unsectionedSortOrder } : {}),
