...
import { sanitizeJournalHtml } from '@/lib/journal/utils';
...
export default async function PublicJournalEntryPage({
  ...
  const { creator, entry } = data;
  const pageUrl = `${process.env.NEXT_PUBLIC_APP_URL || 'https://foleio.com'}/creator/${creator.username}/journal/${entry.slug}`;
  const cleanContent = sanitizeJournalHtml(entry.content || '');
  console.log('[journal-public] Entry content length:', entry.content?.length ?? 0);
